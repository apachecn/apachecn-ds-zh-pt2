...
4.  使用Pandas`scatter_matrix()`功能绘制类似的图，并在对角线上请求内核密度估计图:
...
首先，您需要安装了 ggplot2 的 R。如果你不打算认真使用 ggplot2，也许你应该完全跳过这个食谱。R 的主页是[http://www.r-project.org/](http://www.r-project.org/)(2015 年 7 月检索)。ggplot2 的文件位于[http://docs.ggplot2.org/current/index.html](http://docs.ggplot2.org/current/index.html)(2015 年 7 月检索)。你可以用 pip 安装 pyg plot——我用的是pyg plot-23。要安装`joblib`，请访问[https://pythonhosted.org/joblib/installing.html](https://pythonhosted.org/joblib/installing.html)(2015 年 7 月检索)。通过Python我有`joblib` 0.8.4。
...
3.  下面一行用我们创建的Pandas`DataFrame`对象初始化 pyggplot:
