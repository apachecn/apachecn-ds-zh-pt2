...
NumPy 是一个很好的入门包，让你熟悉数据分析中的数组和面向数组的计算。此外，这是学习其他更有效的工具(如Pandas)的基本步骤，我们将在下一章中看到。我们将使用 NumPy 版本 1.9.1。
...
然而，尽管 NumPy 本身并不提供非常高级的数据分析功能，但了解它将帮助您更有效地使用Pandas等工具。这个工具将在下一章讨论。
