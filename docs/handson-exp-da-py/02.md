...
*   确保你有 Python 3。安装在你的电脑上。建议使用Python等 Python 笔记本。
...
在前面的示例中，我们假设数据以 CSV 格式提供。在现实场景中，数据大多以 CSV、JSON、Excel 或 XML 格式提供，并且大多通过一些标准的应用编程接口传播。对于本系列，我们假设您已经熟悉Pandas以及如何读取不同类型的文件。如果没有，是时候修改Pandas了。更多详情请参考Pandas文档:[https://pandas-datareader.readthedocs.io/en/latest/](https://pandas-datareader.readthedocs.io/en/latest/)。
