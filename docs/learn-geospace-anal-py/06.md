...
在遥感中，我们遍历图像中的每个像素，并执行某种形式的查询或数学处理。图像可以被认为是一个大的数字阵列。在遥感中，这些阵列可能相当大，大小在几十兆字节到几千兆字节的数量级。虽然 Python 速度很快，但只有基于 C 的库才能提供以可容忍的速度遍历数组所需的速度。
...
![](Images/49b6f836-41c4-4c32-ae3a-ae7c8b75071c.png)
...
我们在[第 4 章](04.html)、*地理空间 Python 工具箱*的*安装 GDAL 和 NumPy* 部分安装了带有 Python 绑定的 GDAL 库。GDAL 库包括一个名为`gdal_array`的模块，该模块将遥感图像加载到 NumPy 阵列并保存，以便于操作。GDAL 本身是一个数据访问库，并没有以处理的名义提供太多。因此，在这一章中，我们将严重依赖 NumPy 来实际更改图像。
...
![](Images/d57d2ccf-ba6e-4595-a858-620f2869675f.png)
...
![](Images/ccabeb94-8ddb-4c33-be02-0847abd958cf.png)
...
![](Images/54b4b7e2-a5b7-42b4-92ba-981a15b4bb8c.png)
...
![](Images/53cfa72c-2bc1-49bb-aba8-7402f1557e7c.png)
...
![](Images/ecc2138e-13f1-4111-abec-fe8252abe7e8.png)
...
分析师很少对整个卫星场景感兴趣，因为它可以轻松覆盖数百平方英里。鉴于卫星数据的规模，我们非常有动力将图像缩小到我们感兴趣的区域。实现这一缩减的最佳方法是将图像裁剪到定义我们研究区域的边界。我们可以使用 shapefiles(或其他矢量数据)作为我们的边界定义，并基本上消除该边界之外的所有数据。

以下图像包含我们的`stretched.tif`图像，上面分层有一个县边界文件，在**量子地理信息系统** ( **量子地理信息系统**)中可视化:

![](Images/c175ce35-001f-41a4-a87a-c9e164b6f95e.png)

要剪切图像，我们需要遵循以下步骤:

1.  使用`gdal_array`将图像加载到数组中。
2.  使用 PyShp 创建一个 shapefile 读取器。
3.  将形状文件光栅化为地理参考图像(将其从矢量转换为光栅)。
...
![](Images/c79fbf2a-441a-4cf1-856b-2c65d9aa1f65.png)
...
![](Images/e1a8105d-059b-43a4-ad0c-ba5d31674e10.png)
...
![](Images/a2271208-f024-4758-893d-4a53fde9e4f5.png)
...
现在，我们已经能够分离图像中的要素，我们可以尝试将要素提取为矢量数据，以包含在地理信息系统中。

# 从图像中提取特征

对图像进行分类的能力将我们引向另一种遥感能力。既然你已经在过去的几章中使用了 shapefiles，你有没有想过它们是从哪里来的？形状文件等矢量地理信息系统数据通常是从遥感图像中提取的，例如我们到目前为止看到的例子。
...
![](Images/bdd03cfc-b79b-424a-b888-f7f571fbb1f2.png)
...
![](Images/ae499dec-3998-48cc-ad4d-5ec670e52f70.png)
...
3.  接下来，我们用`gdal_array`将两幅图像读入 NumPy 阵列:
...
![](Images/79fec589-c670-41f0-8612-98194620107b.png)
...
![](Images/b9a17df0-71c6-4c87-9370-b6b7a4f7f56e.png)

您可以将这种变化检测分析与要素提取示例相结合，将变化提取为可在地理信息系统中有效分析的矢量数据。
