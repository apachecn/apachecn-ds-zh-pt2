...
Pandas 是一个高性能的 Python 数据分析库，可以处理大型数据集，包括表格(类似于数据库)、有序/无序、标记矩阵或未标记的统计数据。GeoPandas 只是Pandas的地理空间扩展，它建立在 Shapely、Fiona、PyProj、Matplotlib 和笛卡尔的基础上，所有这些都必须安装。它允许您用 Python 轻松地执行操作，否则需要空间数据库，如 PostGIS。你可以从[http://www.lfd.uci.edu/~gohlke/pythonlibs/#panda](http://www.lfd.uci.edu/~gohlke/pythonlibs/#panda)下载一个地球 andas 的轮子文件。
