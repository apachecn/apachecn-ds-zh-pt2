...
似乎没有很多缺失的价值，但这就是问题所在。当我们使用 head 语句检查前几行时，我们看到一些丢失的值在原始数据集中被表示为 *None* 。然而，Pandas并不认为这些是空值。让我们用值 *np.nan* 替换值 *None* ，这样Pandas就可以将这些值视为空值:
...
我们看到包含数值的列没有被识别为数值列，包含日期的列也没有被正确识别。具有不正确数据类型的列成为下一步的障碍，在下一步中，我们分析趋势并绘制图表；这一步要求列的数据类型采用适合Pandas阅读的格式。
