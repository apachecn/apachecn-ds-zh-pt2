...
这里， *plt* 是我们用来绘图的 Matplotlib 的 *pyplot* 模块的简称或者别名， *sns* 是 Seaborn 库的别名， *pd* 是Pandas的别名。
...
4.  Pandas里用来绘图的 *plot* 函数，在后端使用 Matplotlib。这个函数通过改变传递给它的参数就可以很容易地绘制任何类型的图形，从而利用了多态原理(一个名称，多种形式)。
